...
class OneHotClfr(TrainInterface):
    ...
    def upload(self, i):      
        ...
        with self.mlContext.engine.begin() as connection:
            df.to_sql(
                name='prediciions_categorical',
                con=connection,
                if_exists='append',
                index=False,
            )
        #con=self.mlContext.engine.connect()
        #df.to_sql("prediciions_categorical", con = con, index = False, if_exists='append')
        #con.close()
